  ...
  try {
    ...
    if (Array.isArray(uniqueData)) {
      for (const a of uniqueData) {
        console.log(a.nama);
        if (a.no_telpon != "0") {
          const result = await sendMessageWaGts("Kemiling", a);
