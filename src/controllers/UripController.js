...
const { connectionUrip, connectionMemsys } = require("../config/Database.js");
const {
  getDataGroup,
  sendMessageWa,
  sendMessage,
...
const getTransaksi = async (req, res) => {
  ...
  try {
    const [transaksiPasien] = await connectionUrip.query(queryTransaksiPasien);
...
const sendWa = async (req, res) => {
  try {
    ...
    const response = await fetch(`http://202.157.189.177:5000/urip/transaksi`);
    ...
    if (Array.isArray(uniqueData)) {
      for (const a of uniqueData) {
        console.log(a.nama);
        if (a.no_telpon != "0") {
          const result = await sendMessageWa("Urip", a);
