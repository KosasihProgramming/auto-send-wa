...
const getTransaksi = async (req, res) => {
  ...
  try {
    const [transaksiPasien] = await connectionGtsTirtayasa.query(
      queryTransaksiPasien
    );
    const [dataPasien] = await connectionMemsysGtsTirtayasa.query(
      queryMasterPasien
    );
...
const sendWa = async (req, res) => {
  try {
    ...
    const response = await fetch(
      `http://202.157.189.177:5000/gts-tirta/transaksi`
    );
