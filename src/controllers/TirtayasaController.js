...
const getTransaksi = async (req, res) => {
  ...
  try {
    const [transaksiPasien] = await connectionTirtayasa.query(
...
const sendWa = async (req, res) => {
  try {
    ...
    const response = await fetch(
      `http://202.157.189.177:5000/tirtayasa/transaksi`
    );
    ...
    if (Array.isArray(uniqueData)) {
      for (const a of uniqueData) {
        console.log(a.nama);
        if (a.no_telpon != "0") {
          const result = await sendMessageWa("Tirtayasa", a);
